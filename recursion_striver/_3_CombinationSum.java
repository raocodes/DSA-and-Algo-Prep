...
public class _3_CombinationSum {
    public void findCombinations(int index, int target, int[] candidates, List<Integer> sequence,
        ...
        if (candidates[index] <= target) {
            ...
            findCombinations(index, target - candidates[index], candidates, new ArrayList<>(sequence), result);
            sequence.remove(sequence.size() - 1);
        }
        findCombinations(index + 1, target, candidates, new ArrayList<>(sequence), result);
    }
